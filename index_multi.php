...
?>
			<tr style="line-height:30px;cursor:pointer;" >
			  <th scope="row" onclick="move_page('<?=urlencode($p->_source->url)?>','<?=$p->num?>')"><?=$p->_source->site_cnt?></th>
			  <td><a href="link.php?url=<?=urlencode($p->_source->url)?>&num=<?=$p->num?>" target="_blank"><?=$p->_source->subject?></a> </td>
			  <td onclick="move_page('<?=urlencode($p->_source->url)?>','<?=$p->num?>')"><?echo site_name_is($p->_source->site_name);?></td>
			</tr>
