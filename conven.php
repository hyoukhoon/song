    ...
    <div class="container">
		...
		<div >
			<p  style="text-align:center;">
				<input type="radio" name="isevent" value="" <?if(!$isevent){?>checked<?}?> onclick="location.href='/conven.php'">전체 / 
				<input type="radio" name="isevent" value="1" <?if($isevent=="1"){?>checked<?}?> onclick="location.href='/conven.php?isevent=1'">1+1행사 / 
				<input type="radio" name="isevent" value="2" <?if($isevent=="2"){?>checked<?}?> onclick="location.href='/conven.php?isevent=2'">2+1행사
			</p>
		</div>
	  ...
	  <table class="table table-sm">
		  <thead class="thead-light">
			<tr>
			  <th scope="col" style="width:80px;">EVENT</th>
			  <th scope="col">상품명</th>
...
if($isevent){
	if($isevent==1){
		$where=" and etc1='1+1행사'";
	}else if($isevent==2){
		$where=" and etc1='2+1행사'";
	}
}
...
if($sword){
	...
	$que2="select count(1) from every_board where multi='conven' and left(reg_date,10)='$now2' $where";
...
$que="SELECT * FROM every_board where multi='conven' and left(reg_date,10)='$now2'  $where $ord  limit $start_page, $end_page";
...
?>
			<tr style="line-height:30px;cursor:pointer;">
			  <th scope="row"><?echo $rs->etc1;?></th>
			  <td><img src="<?echo $rs->imgurl;?>" style="max-height:150px;padding-right:10px;" align="left"><?echo $rs->subject;?>
			  <p style="font-weight:bold;"><?echo number_format($rs->price)."원";?></p></td>
			  <td><?echo $rs->site_name;?></td>
			</tr>
...
function move_page(url,uid){
      ...
      <ul class="pagination justify-content-center">
        ...
        <li class="page-item">
			<?if($f_no!=1){?>
          <a class="page-link" href="<?=$_SERVER['PHP_SELF']?>?mode=<?=$mode?>&page=<?=$p_f_no?>&f_no=<?=$p_f_no?>&gubun=<?=$gubun?>&ord=<?=$ord?>&s_key=<?=$s_key?>&s_word=<?=$s_word?>&start_date=<?=$start_date?>&end_date=<?=$end_date?>&STS_CODE=<?=$STS_CODE?>&ste=<?=$ste?>&m1=<?=$m1?>&m2=<?=$m2?>&isevent=<?=$isevent?>" aria-label="Previous">
		...
		<? for($i=$f_no;$i<=$l_no;$i++){?>
					...
					<?} else {?>
						<li class="page-item">
						  <a class="page-link" href="<?=$PHP_SELF?>?mode=<?=$mode?>&page=<?=$i?>&f_no=<?=$f_no?>&gubun=<?=$gubun?>&ord=<?=$ord?>&s_key=<?=$s_key?>&s_word=<?=$s_word?>&start_date=<?=$start_date?>&end_date=<?=$end_date?>&STS_CODE=<?=$STS_CODE?>&ste=<?=$ste?>&m1=<?=$m1?>&m2=<?=$m2?>&isevent=<?=$isevent?>"><?=$i?></a>
						</li>
					<?}?>
				<?}?>
        ...
        <li class="page-item">
			<?if($l_no<$total_page){?>
          <a class="page-link" href="<?=$_SERVER['PHP_SELF']?>?mode=<?=$mode?>&page=<?=$n_f_no?>&f_no=<?=$n_f_no?>&gubun=<?=$gubun?>&ord=<?=$ord?>&s_key=<?=$s_key?>&s_word=<?=$s_word?>&start_date=<?=$start_date?>&end_date=<?=$end_date?>&STS_CODE=<?=$STS_CODE?>&ste=<?=$ste?>&m1=<?=$m1?>&m2=<?=$m2?>&isevent=<?=$isevent?>" aria-label="Next">
