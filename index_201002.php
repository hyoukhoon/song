...
if($orderby==1){
	$ord="order by a.site_reg_date desc";
}else if($orderby==2){
	$ord="order by a.site_cnt desc";
}else{
	$ord="order by a.site_reg_date desc";
}
...
if($gubun=="review" or $gubun=="info"){
	...
	$que2="SELECT count(1) FROM every_board a inner join (select * from every_board where site_reg_date > DATE_ADD(NOW(), INTERVAL -168 HOUR)) b on a.num=b.num where a.multi in ('free','sisa','humor','star','review','black') and a.site_reg_date<='$today_date' $where";

}else{
		$que2="SELECT count(1) FROM every_board a inner join (select * from every_board where site_reg_date > DATE_ADD(NOW(), INTERVAL -24 HOUR)) b on a.num=b.num where a.multi in ('free','sisa','humor','star','review','black') and a.site_reg_date<='$today_date' $where";
}
...
if($gubun=="review" or $gubun=="info"){
	$que="SELECT * FROM every_board a inner join (select * from every_board where site_reg_date > DATE_ADD(NOW(), INTERVAL -168 HOUR)) b on a.num=b.num where a.multi in ('free','sisa','humor','star','review','black') and a.site_reg_date<='$today_date'  $where $ord  limit $start_page, $end_page";
}else{
	$que="SELECT * FROM every_board a inner join (select * from every_board where site_reg_date > DATE_ADD(NOW(), INTERVAL -24 HOUR)) b on a.num=b.num where a.multi in ('free','sisa','humor','star','review','black') and a.site_reg_date<='$today_date'  $where $ord  limit $start_page, $end_page";
}
//echo $que."<br>";
