...
<input type="hidden" name="gubun" value="<?=$gubun?>">
	  ...
	  <table class="table table-sm">
		  ...
		  <!-- <tr style="line-height:30px;cursor:pointer;" >
			  <th scope="row">1</th>
			  <td><a href="https://search.naver.com/search.naver?sm=top_hty&fbm=0&ie=utf8&query=%ED%94%84%EB%A1%9C%ED%94%BD" target="_blank">틀리면 100%환불+10%위로금까지 프로픽만의 환불보상제 실시</a></td>
			  <td><a href="https://search.naver.com/search.naver?sm=top_hty&fbm=0&ie=utf8&query=%ED%94%84%EB%A1%9C%ED%94%BD" target="_blank">프로픽</a></td>
			</tr> -->
...
if($gubun=="black" or $gubun=="info"){

	$que2="select count(1) from every_board where multi in ('free','sisa','humor','star','review','black') and site_reg_date > DATE_ADD(now(), INTERVAL -336 HOUR) $where";

}else{
		$que2="select count(1) from every_board where multi in ('free','sisa','humor','star','review','black') and site_reg_date > DATE_ADD(now(), INTERVAL -24 HOUR) $where";
}
...
/*
if($gubun=="black" or $gubun=="info"){
	$que="SELECT * FROM every_board where multi in ('free','sisa','humor','star','review','black') and site_reg_date > DATE_ADD(now(), INTERVAL -336 HOUR) and site_reg_date<='$today_date'  $where $ord  limit $start_page, $end_page";
}else{
	$que="SELECT * FROM every_board where multi in ('free','sisa','humor','star','review','black') and site_reg_date > DATE_ADD(now(), INTERVAL -24 HOUR) and site_reg_date<='$today_date'  $where $ord  limit $start_page, $end_page";
}
*/

if($gubun=="black" or $gubun=="info"){
	$que="SELECT * FROM every_board where multi in ('free','sisa','humor','star','review','black') and site_reg_date > DATE_ADD(now(), INTERVAL -336 HOUR)  $where $ord  limit $start_page, $end_page";
}else{
	$que="SELECT * FROM every_board where multi in ('free','sisa','humor','star','review','black') and site_reg_date > DATE_ADD(now(), INTERVAL -24 HOUR)  $where $ord  limit $start_page, $end_page";
}
//echo $que."<br>";
...
?>
			<tr style="line-height:30px;cursor:pointer;" >
			  <th scope="row" onclick="move_page('<?=urlencode($url)?>','<?=$rs->num?>')"><?echo substr($rs->site_reg_date,0,10);?></th>
