...
function site_name_is($n){
		...
		switch($n) {
			...
			break;
			case "shez":$rs="<font color='#000000'>쉬즈</font>";
			break;
			case "bali":$rs="<font color='#aa8000'>발리</font>";
			break;
			case "love":$rs="<font color='#008055'>러브미</font>";
			break;
			case "play":$rs="<font color='#bb0055'>플레이</font>";
			break;
			case "soul":$rs="<font color='#bbaa55'>소울</font>";
			break;
			...
			break;
			case "conella":$rs="<font color='#dd11cc'>코넬라</font>";
			break;
			case "comeon":$rs="<font color='#aa55aa'>컴온</font>";
			break;
...
$que2="select count(1) from every_board where site_name in ('hnhn','dorosi','thezam','bbongbra','conella','comeon','play','soul','love','bali','shez') and site_reg_date > DATE_ADD(now(), INTERVAL -168 HOUR) and site_reg_date<='$today_date' $where";
...
$no=$total-($page-1)*$ps;//번호매기기
	...
	$que="SELECT * FROM every_board where site_name in ('hnhn','dorosi','thezam','bbongbra','conella','comeon','play','soul','love','bali','shez') and site_reg_date > DATE_ADD(now(), INTERVAL -168 HOUR) and site_reg_date<='$today_date'  $where $ord  limit $start_page, $end_page";
...
?>
			<tr style="line-height:30px;cursor:pointer;" >
			  ...
			  <td><?if($rs->site_name=="hnhn" or $rs->site_name=="dorosi" or $rs->site_name=="thezam" or $rs->site_name=="bbongbra" or $rs->site_name=="conella" or $rs->site_name=="comeon"){?>[후방]<?}?><a href="link.php?url=<?=urlencode($url)?>&num=<?=$rs->num?>" target="_blank"><?echo stringcututf($rs->subject,35,"..");?></a> </td>
...
$(function() {
		...
		timer = setInterval( function () {
			...
			$.ajax ({
				"url" : "./realtime/clicks.php",
				cache : false,
				success : function (html) { 
					$("#clicks").text("Today Click : "+html);
				}
			});
